...
/**
 * Wiki7ComponentSiteStats component
 * FIXME: Need unit test
 */
class Wiki7ComponentSiteStats implements Wiki7Component {
	...
	public function getTemplateData(): array {
		$config = $this->config;
		if ( !$config->get( 'Wiki7EnableDrawerSiteStats' ) ) {
			return [];
		}
		...
		if ( $config->get( 'Wiki7UseNumberFormatter' ) && class_exists( NumberFormatter::class ) ) {
		...
		foreach ( self::SITESTATS_ICON_MAP as $key => $icon ) {
			$items[] = [
				...
				'label' => $this->localizer->msg( "wiki7-sitestats-$key-label" )->text(),
			];
		}
