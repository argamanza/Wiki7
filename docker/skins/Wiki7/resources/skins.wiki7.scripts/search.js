...
function bindSearchTrigger( document, mw, details ) {
	document.querySelectorAll( '.wiki7-search-trigger' ).forEach( ( trigger ) => {
		trigger.addEventListener( 'click', ( event ) => {
			openSearch( details );
			if ( event.target.dataset.wiki7SearchPrefill ) {
				// Add a delay to ensure the search UI is open
				setTimeout( () => {
					const input = config.wgWiki7EnableCommandPalette ?
						document.querySelector(
							'.wiki7-command-palette__input > .cdx-text-input__input'
						) :
					...
					input.value = mw.html.escape(
						event.target.dataset.wiki7SearchPrefill
					);
				}, 0 );
			}
		} );
...
/**
 * Initializes the search functionality for the Wiki7 search boxes.
 *
 * @param {Object} deps
...
function initSearch( { window, document, mw } ) {
	...
	const searchBoxes = document.querySelectorAll( '.wiki7-search-box' );
	...
	searchBoxes.forEach( ( searchBox ) => {
		...
		loadSearchModule( document, mw, input, config.wgWiki7SearchModule, () => {
