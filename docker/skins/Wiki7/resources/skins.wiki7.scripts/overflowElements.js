...
class OverflowElement {
	...
	updateState() {
		...
		window.requestAnimationFrame( () => {
			const updateClasses = [
				[ isLeftOverflowing, 'wiki7-overflow--left' ],
				[ isRightOverflowing, 'wiki7-overflow--right' ]
			];
			this.toggleClasses( updateClasses );
			if ( this.stickyHeader ) {
				this.stickyHeader.style.setProperty( '--wiki7-overflow-scroll-x', this.contentScrollLeft + 'px' );
			}
		} );
	}
	...
	handleInheritedClasses() {
		const inheritedClasses = config.wgWiki7OverflowInheritedClasses;
	...
	/**
	 * Wraps the element in a div container with the class 'wiki7-overflow-wrapper'.
	 ...
	 * Creates a new div wrapper element, adds the class 'wiki7-overflow-wrapper', and appends it to the parent node before the element.
	...
	wrap() {
		if ( !this.element || !this.element.parentNode ) {
			mw.log.error(
				'[Wiki7] Element or element.parentNode is null or undefined. Please check if the element or element.parentNode is null or undefined.'
			);
			return;
		}
		try {
	...
	setupStickyHeader() {
		...
		this.stickyHeader.classList.add( 'wiki7-overflow-content-sticky-header' );
	...
	onClick( event ) {
		...
		if ( !target.classList.contains( 'wiki7-overflow-navButton' ) ) {
			return;
		}
		// Prevent triggering the form submit action (e.g. realtime preview in WikiEditor)
		event.preventDefault();
		const offset = this.wrapper.offsetWidth / 2;
		if ( target.classList.contains( 'wiki7-overflow-navButton-left' ) ) {
			this.scrollContent( -offset );
		} else if (
			target.classList.contains( 'wiki7-overflow-navButton-right' )
		) {
			this.scrollContent( offset );
		}
	}
...
function init( bodyContent ) {
	const nowrapClasses = config.wgWiki7OverflowNowrapClasses;
	if ( !nowrapClasses || !Array.isArray( nowrapClasses ) ) {
		mw.log.error(
			'[Wiki7] Invalid or missing $wgWiki7OverflowNowrapClasses. Cannot proceed with wrapping element.'
		);
		return;
	}

	const overflowElements = bodyContent.querySelectorAll(
		'.wiki7-overflow, .wikitable:not( .wikitable .wikitable )'
	);
