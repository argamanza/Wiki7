...
function init() {
	...
	const alertIcon = notifications.querySelector( '#pt-notifications-alert > a > .wiki7-ui-icon' ).cloneNode();
	const noticeIcon = notifications.querySelector( '#pt-notifications-notice > a > .wiki7-ui-icon' ).cloneNode();
	...
	const setupFosterHome = ( badge, anchor ) => {
		...
		anchor.classList.add( 'wiki7-header__button', 'wiki7-echo-notification-badge' );
		...
		for ( const mutation of mutations ) {
			if ( mutation.type === 'childList' ) {
				...
				for ( const removedNode of removedNodes ) {
					if ( !( removedNode instanceof HTMLSpanElement ) || !removedNode.classList.contains( 'wiki7-ui-icon' ) ) {
						return;
					}
