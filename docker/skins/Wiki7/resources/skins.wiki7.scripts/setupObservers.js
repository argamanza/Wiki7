...
const
	...
	SCROLL_DOWN_CLASS = 'wiki7-scroll--down',
	SCROLL_UP_CLASS = 'wiki7-scroll--up',
	PAGE_TITLE_INTERSECTION_CLASS = 'wiki7-below-page-title';
...
const main = () => {
	...
	const
		...
		stickyIntersection = document.getElementById( 'wiki7-page-header-sticky-sentinel' ),
	...
	const resumeStickyHeader = () => {
		if (
			...
			if ( document.documentElement.classList.contains( 'wiki7-feature-autohide-navigation-clientpref-1' ) ) {
	...
	const bodyObserver = resizeObserver.initResizeObserver(
		...
		( entry ) => {
			...
			if ( document.documentElement.classList.contains( 'wiki7-animations-ready' ) ) {
				document.documentElement.classList.remove( 'wiki7-animations-ready' );
			}
		},
		// onResizeEnd
		( entry ) => {
