...
const RECENT_ITEMS_KEY = 'skin-wiki7-command-palette-recent-items';
...
function createRecentItems() {
	...
	function saveSearchQuery( query, searchUrl ) {
		saveRecentItem( {
			...
			id: `wiki7-command-palette-result-search-${ mw.util.escapeIdForAttribute( query ) }`,
			label: query,
			url: searchUrl,
	...
	function getRecentItems() {
		...
		const dismissAction = {
			id: 'dismiss',
			label: mw.msg( 'wiki7-command-palette-dismiss' ),
			icon: cdxIconTrash
		};
