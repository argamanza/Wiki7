...
<template>
	<li
		...
		class="wiki7-command-palette-list-item"
		:class="rootClasses"
		:data-type="type"
...
module.exports = exports = defineComponent( {
	...
	setup( props, { emit, expose } ) {
		...
		const rootClasses = computed( () => ( {
			'wiki7-command-palette-list-item--active': props.active && props.highlighted,
			'wiki7-command-palette-list-item--highlighted': props.highlighted
		} ) );

		// Messages that can be used here:
		// * wiki7-command-palette-type-page
		// eslint-disable-next-line mediawiki/msg-doc
		const typeLabel = computed( () => mw.message( `wiki7-command-palette-type-${ props.type }` ).text() );
