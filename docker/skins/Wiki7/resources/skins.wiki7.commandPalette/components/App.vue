<template>
	...
	>
		...
		<div
			...
			class="wiki7-command-palette__results"
		>
			...
			<template v-else-if="searchStore.searchQuery && !searchStore.isPending && displayedItems.length === 0">
				<command-palette-empty-state
					:title="$i18n( 'wiki7-search-noresults-title' ).params( [ searchStore.searchQuery ] ).text()"
...
module.exports = exports = defineComponent( {
	...
	setup() {
		...
		const handleRootKeydown = ( event ) => {
			...
			const isActionFocused = event.target?.closest( '.wiki7-command-palette-list-item__action' );
