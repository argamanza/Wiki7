<template>
	<div
		...
		class="wiki7-command-palette-list-item__actions"
		:class="{ 'wiki7-command-palette-list-item__actions--visible': highlighted }"
	>
		<cdx-button
			v-for="action in actions"
			:key="action.id"
			:ref="( el ) => setButtonRef( el, action.id )"
			class="wiki7-command-palette-list-item__action"
		...
		>
			<cdx-icon
				:icon="action.icon"
				size="small"
				class="wiki7-command-palette-list-item__action__icon"
			></cdx-icon>
		</cdx-button>
	</div>
</template>
...
const { CdxIcon, CdxButton } = mw.loader.require( 'skins.wiki7.commandPalette.codex' );
...
.wiki7-command-palette-list-item {
	&__actions {
		position: absolute;
		inset-inline-end: var( --wiki7-command-palette-side-padding );
		top: 0;
		display: flex;
		gap: var( --space-xxs );
		align-items: center;
