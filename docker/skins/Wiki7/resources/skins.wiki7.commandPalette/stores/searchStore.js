...
const FULLTEXT_SEARCH_DESCRIPTION = mw.message( 'wiki7-command-palette-type-fulltext-search-description' ).text();
	...
	actions: {
		...
		handleSyncProvider( provider, query ) {
			try {
				...
				mw.log.error( `[skins.wiki7.commandPalette] Sync Provider failed for query "${ query }":`, error );
				if ( this.searchQuery === query ) {
		...
		handleAsyncProvider( provider, query ) {
			...
			this.debounceTimeout = setTimeout( async () => {
				...
				try {
					...
					mw.log.error( `[skins.wiki7.commandPalette] Async Provider failed for query "${ query }":`, error );
					if ( this.searchQuery === query ) {
						this.setProviderResults( [], 'error' ); // Clear results on error
					}
				} finally {
		...
		async fetchRelatedArticles() {
			try {
				...
				mw.log.error( '[skins.wiki7.commandPalette] Failed to get related articles:', error );
				return [];
			}
		},
		...
		fetchRecentItems() {
			try {
				...
				mw.log.error( '[skins.wiki7.commandPalette] Failed to get recent items:', error );
				return [];
			}
		},
		...
		async handleSelection( result ) {
			...
			if ( sourceProvider && typeof sourceProvider.onResultSelect === 'function' ) {
				try {
					...
					mw.log.error( `[skins.wiki7.commandPalette|searchStore] Error handling selection for source: ${ result.source }`, error );
					return { action: 'none' };
				}
			} else {
				mw.log.warn( `[skins.wiki7.commandPalette|searchStore] No provider or onResultSelect found for source: ${ result.source }`, result );
