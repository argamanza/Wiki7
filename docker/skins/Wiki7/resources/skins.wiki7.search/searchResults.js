...
/**
 * Returns an object with methods for handling search results in a wiki7 search context.
	...
	return {
		getRedirectLabel: function ( title, matchedTitle, queryValue ) {
			...
			const generateRedirectHtml = () => {
				...
				div.classList.add( 'wiki7-typeahead__labelItem' );
				div.title = getRedirectMessage();

				const spanIcon = document.createElement( 'span' );
				spanIcon.classList.add( 'wiki7-ui-icon', 'mw-ui-icon-wikimedia-articleRedirect' );
		...
		getPlaceholderHTML: function ( queryValue, templates ) {
			const data = {
				...
				title: mw.message( 'wiki7-search-noresults-title', queryValue ).text(),
				description: mw.message( 'wiki7-search-noresults-desc' ).text()
			};
		...
		getResultsHTML: function ( results, queryValue, templates ) {
			...
			results.forEach( ( result, index ) => {
				...
				if ( result.thumbnail && result.thumbnail.url ) {
					...
					item.image.class = 'wiki7-typeahead-list-item-image--placeholder wiki7-ui-icon mw-ui-icon-wikimedia-image';
				}
				items.push( item );
			} );
		...
		clear: function () {
			...
			document.getElementById( 'wiki7-typeahead-list-page' ).innerHTML = '';
			document.getElementById( 'wiki7-typeahead-group-page' ).hidden = true;
			searchAction.clear();
		},
		init: function () {
			searchAction.init();
