		...
		render: function ( searchQuery, templates ) {
			...
			items.push( {
				...
				text: mw.message( config.isAdvancedSearchExtensionEnabled ?
					'wiki7-search-advancedsearch' :
					'wiki7-search-fulltext'
				)
			} );
			...
			if ( config.isMediaSearchExtensionEnabled ) {
				...
				items.push( {
					...
					icon: 'imageGallery',
					text: mw.message( 'wiki7-search-mediasearch' )
				} );
			}
			...
			*/
			...
			if ( this.userCanEdit ) {
				...
				items.push( {
					...
					icon: 'edit',
					text: mw.message( 'wiki7-search-editpage' )
				} );
			}
			...
			document.getElementById( 'wiki7-typeahead-list-action' ).outerHTML = templates.TypeaheadList.render( data, partials ).html();
			document.getElementById( 'wiki7-typeahead-group-action' ).hidden = false;
		},
		clear: function () {
			document.getElementById( 'wiki7-typeahead-list-action' ).innerHTML = '';
			document.getElementById( 'wiki7-typeahead-group-action' ).hidden = true;
		},
		init: function () {
