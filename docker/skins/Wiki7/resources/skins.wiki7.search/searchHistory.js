...
function searchHistory( config ) {
	return {
		data: [],
		limit: config.wgWiki7MaxSearchResults,
		...
		set: function ( arr ) {
			if ( arr && this.isValidSearchHistory( arr ) ) {
				this.data = arr;
				mw.storage.set( 'skin-wiki7-search-history', JSON.stringify( this.data ) );
			}
		},
		add: function ( query ) {
		...
		clear: function () {
			this.set( [] );
			mw.storage.remove( 'skin-wiki7-search-history' );
		},
		init: function () {
			const storedData = mw.storage.get( 'skin-wiki7-search-history' );
			if ( storedData ) {
				try {
					this.data = JSON.parse( storedData );
				} catch ( error ) {
					mw.log.error( `[Wiki7] Error parsing search history. Stored data: ${ storedData }`, error );
				}
			}
		}
	};
