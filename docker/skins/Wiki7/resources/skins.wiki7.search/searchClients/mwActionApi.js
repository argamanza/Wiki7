...
function adaptApiResponse( config, query, response, showDescription ) {
	...
	const getDescription = ( page ) => {
		switch ( config.wgWiki7SearchDescriptionSource ) {
			case 'wikidata':
	...
	if ( response.redirects ) {
		...
		// Sometimes there can be multiple redirect object for the same page, only take the
		// one with lower index
...
function mwActionApiSearchClient( config ) {
	return {
		 ...
		 * @type {fetchByTitle}
		 */
		fetchByTitle: ( q, limit = config.wgWiki7MaxSearchResults, showDescription = true ) => {
			const cacheExpiry = config.wgSearchSuggestCacheExpiry;
			const descriptionSource = config.wgWiki7SearchDescriptionSource;
