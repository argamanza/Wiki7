const SEARCH_LOADING_CLASS = 'wiki7-loading';

// Config object from getWiki7SearchResourceLoaderConfig()
...
const typeahead = {
	...
	input: {
		...
		init: function ( inputEl ) {
			...
			wrapper.classList.add( 'wiki7-typeahead-input-group' );
			typeaheadInputElement.parentNode.insertBefore( wrapper, typeaheadInputElement );

			const overlay = document.createElement( 'div' );
			overlay.classList.add( 'wiki7-typeahead-input-overlay' );
			this.displayElement = document.createElement( 'span' );
			this.displayElement.textContent = typeaheadInputElement.value;
			this.displayElement.classList.add( 'wiki7-typeahead-input-overlay-query' );
			overlay.append( this.displayElement );

			typeaheadInputElement.classList.add( 'wiki7-typeahead-input' );
		...
		onFocus: function () {
			...
			typeahead.form.element.parentElement.classList.add( 'wiki7-search__card--expanded' );
	...
	items: {
		...
		/**
		 * Sets 'wiki7-typeahead__item--active' class on the element
		 *
		...
		set: function () {
			...
			this.elements = typeaheadElement.querySelectorAll( '.wiki7-typeahead-group[data-mw-typeahead-keyboard-navigation] .wiki7-typeahead-list-item-link' );
	...
	onBlur: function ( event ) {
		...
		if ( !typeaheadElement.contains( event.relatedTarget ) ) {
			...
			setTimeout( () => {
				typeahead.form.element.parentElement.classList.remove( 'wiki7-search__card--expanded' );
	...
	onClick: function ( event ) {
		...
		if ( typeahead.element.contains( event.target ) ) {
			const link = event.target.closest( '.wiki7-typeahead-list-item-link' );
			// Early exit if target is not a link
			if ( !link ) {
				return;
			}

			const group = event.target.closest( '.wiki7-typeahead-group' );
			...
			if ( historyType ) {
				...
				if ( historyType === 'query' ) {
					...
					const historyTextEl = link.querySelector( `.wiki7-typeahead-list-item-${ historyType }` );
	...
	updateSearchClient: function () {
		const typeaheadInputElement = typeahead.input.element;
		searchClient.setActive( config.wgWiki7SearchGateway );
	...
	init: function ( formEl, inputEl ) {
		...
		const data = {
			...
			'array-lists': [
				{ type: 'action', class: 'wiki7-typeahead-group--chips', hidden: true, historyValue: 'query' },
...
async function getSuggestions() {
	...
	const renderSuggestions = ( results ) => {
		const groupEl = document.getElementById( 'wiki7-typeahead-group-page' );
		const listEl = document.getElementById( 'wiki7-typeahead-list-page' );
