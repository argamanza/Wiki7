...
function addDefaultPortlet( portlet ) {
	...
	ul.classList.add( 'wiki7-menu__content-list' );
	const label = portlet.querySelector( 'label' );
	if ( label ) {
		const labelDiv = document.createElement( 'div' );
		labelDiv.classList.add( 'wiki7-menu__heading' );
	...
	if ( wrapper ) {
		...
		wrapper.classList.add( 'wiki7-menu__content' );
	} else {
	...
	portlet.classList.add( 'wiki7-menu' );
	return portlet;
}
