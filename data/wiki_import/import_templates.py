...
# ---------------------------------------------------------------------------
# Cargo table definitions for MediaWiki Cargo extension
# ---------------------------------------------------------------------------
...
def _build_cargo_template(table_name: str, fields: dict) -> str:
    ...
    lines = [f"<noinclude>", f"This template defines the Cargo table '''{ table_name }'''.", ""]
...
def import_cargo_templates(
    ...
    """Create/update Cargo table definition templates.

    Args:
        site: An authenticated mwclient.Site instance (None for dry-run).
        dry_run: If True, just preview what would be created/changed.

    Returns:
        Summary dict with page operation counts.
    """
...
def import_transfer_page(
    ...
    """Create/update the season transfer summary page.

    Args:
        site: An authenticated mwclient.Site instance.
        season: Season year string.
        players_path: Path to players.jsonl (for player name lookup).
    ...
    # Filter transfers for this season and classify incoming/outgoing
